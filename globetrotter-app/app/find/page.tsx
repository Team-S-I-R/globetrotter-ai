    ...
    fetch('http://127.0.0.1:5000/plan_trip')
      ...
      .then(data => {
        try {
          ...
          console.error('Error parsing trip info:', error);
          toast({ title: '❌ Error!', description: 'Failed to parse trip information.', itemID: 'error' });
        }
        setShowPopup(true);
      })
      .catch(error => {
                        ...
                        <>
                          ...
                          <motion.div
                            ...
                            <div className="w-full h-full overflow-y-scroll no-scrollbar">
                              {searchedFlights?.flights?.map((flight, index) => (
                                <motion.div
                                  initial={{ opacity: 0, x: 1000 }}
                                  animate={{ opacity: 1, x: 0, transition: { duration: 1.5, staggerChildren: 0.2 } }}
            ...
            {tripInfo && (
              <>
                <div className="mb-4">
                  ...
                  <ul>
                    {Object.entries(tripInfo.itinerary).map(([day, activity]) => (
                      <li key={day}><strong>{day}:</strong> {Object.entries(tripInfo.itinerary).map(([day, activity]) => (
                        <li key={day}><strong>{day}:</strong> {tripInfo.itinerary[day]}</li>
                      ))}
                      </li>
                    ))}
                  </ul>
                ...
                <div className="mb-4">
                  <h3 className="font-bold">Packing List:</h3>
                  <ul>
                    {tripInfo.packing_list.map((item: string, index: number) => (
                      <li key={index}>{item}</li>
                    ))}
                  </ul>
                </div>
                <div className="mb-4">
                  <h3 className="font-bold">Cultural Info:</h3>
                  <p><strong>Cultural Practices:</strong> {tripInfo.cultural_info.Cultural_Practices}</p>
                  <p><strong>Local Cuisine:</strong> {tripInfo.cultural_info.Local_Cuisine}</p>
                  <h4 className="font-bold">Must-See Attractions:</h4>
                  <ul>
                    {tripInfo.cultural_info.Must_See_Attractions.map((attraction: string, index: number) => (
                      <li key={index}>{attraction}</li>
                    ))}
                  </ul>
                </div>
                <div>
                  <h3 className="font-bold">Additional Comments:</h3>
                  <p>{tripInfo.additional_comments}</p>
                </div>
              </>
            )}
            <Button className="mt-4" onClick={closePopup}>Close</Button>
          </div>
        </div>
      )}
    </>
